...
// Grid Settings
When('I change the grid step to {int} in the GUI', async function(this: FloorplanWorld, gridStep: number) {
...
Then('the grid step should be updated to {int}', async function(this: FloorplanWorld, expectedValue: number) {
  const gridStepInput = this.page.getByTestId('grid-step-input');
...
// Adding Rooms
When('I click {string} in the GUI editor', async function(this: FloorplanWorld, buttonText: string) {
...
Then('the room ID should be auto-generated as {string}', async function(this: FloorplanWorld, expectedId: string) {
  const lastRoomId = (this as any).lastRoomId;
  expect(lastRoomId).toBe(expectedId);
});

When('I add another room with name {string}', async function(this: FloorplanWorld, roomName: string) {
...
Then('the second room ID should be {string}', async function(this: FloorplanWorld, expectedId: string) {
...
When('I set width to {int} and depth to {int}', async function(this: FloorplanWorld, width: number, depth: number) {
  const roomId = (this as any).currentRoomId;
  const widthInput = this.page.getByTestId(`room-width-${roomId}`);
  const depthInput = this.page.getByTestId(`room-depth-${roomId}`);
...
When('I select {string} in the attachTo dropdown', async function(this: FloorplanWorld, value: string) {
  const roomId = (this as any).currentRoomId;
  const attachToSelect = this.page.getByTestId(`room-attach-to-${roomId}`);
...
Then('I should be able to set room, wall, width, and offset', async function(this: FloorplanWorld) {
  const windowCard = this.page.getByTestId('window-card-0');
...
Then('options should include inwards-left, inwards-right, outwards-left, outwards-right', async function(this: FloorplanWorld) {
  const swingSelect = this.page.getByTestId('door-swing-0');
...
Then('the GUI editor should scroll to that room\'s configuration', async function(this: FloorplanWorld) {
  expect(true).toBe(true); // TODO: Implement
});
...
Then('parts should be visually grouped under the parent room', async function(this: FloorplanWorld) {
  expect(true).toBe(true); // TODO: Implement
});
