...
When('I create a room with dimensions {int}x{int} attached to Zero Point', async function(this: FloorplanWorld, width: number, depth: number) {
  await this.page.getByTestId('tab-json').click();
...
Given('I have a room attached to Zero Point with size {int}x{int}', async function(this: FloorplanWorld, width: number, depth: number) {
  await this.page.getByTestId('tab-json').click();
  await this.page.waitForTimeout(200);
...
When('I add another room attached to the first with size {int}x{int}', async function(this: FloorplanWorld, width: number, depth: number) {
...
When('I change grid_step from {int} to {int}', async function(this: FloorplanWorld, fromStep: number, toStep: number) {
  await this.page.getByTestId('tab-json').click();
  await this.page.waitForTimeout(200);
...
Then('the room label should display {string}', async function(this: FloorplanWorld, expectedLabel: string) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('the label should be centered within the room rectangle', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('the composite room should appear as one unified shape', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('internal borders between adjacent parts should not be visible', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('the entire composite room should indicate it\'s one unit', async function(this: FloorplanWorld) {
  const compositeRoom = this.page.locator('[data-room-id="composite1"]');
  await expect(compositeRoom).toBeVisible();
});
...
Given('I have a room with {int} room objects', async function(this: FloorplanWorld, objectCount: number) {
  const objects = [];
...
Then('the square should be rendered with correct dimensions', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
});
...
When('I add an object with anchor {string} and roomAnchor {string}', async function(this: FloorplanWorld, anchor: string, roomAnchor: string) {
  const json = {
    grid_step: 1000,
    rooms: [{
      id: 'room1',
...
Then('the object\'s top-left should align with room\'s bottom-right', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
  // Alignment is calculated by renderer
});

Then('the object should be positioned correctly relative to the room', async function(this: FloorplanWorld) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
});
...
When('I create a room with width {int} and depth {int}', async function(this: FloorplanWorld, width: number, depth: number) {
  const json = {
    grid_step: 1000,
    rooms: [{
      id: 'room1',
...
Then('the room should be {int}mm wide', async function(this: FloorplanWorld, expectedWidth: number) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('the room should be {int}mm deep', async function(this: FloorplanWorld, expectedDepth: number) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
...
Then('the display scale should be {int}:{int} \\(1mm = 0.2px\\)', async function(this: FloorplanWorld, scale1: number, scale2: number) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible();
  // Scale is defined in utils.ts DISPLAY_SCALE = 2
});

When('I have {int} rooms and {int} has a positioning error', async function(this: FloorplanWorld, totalRooms: number, errorRooms: number) {
  const rooms = [];
...
Then('the {int} valid rooms should render successfully', async function(this: FloorplanWorld, validRoomCount: number) {
  const svg = this.page.locator('.floorplan-svg');
  await expect(svg).toBeVisible({ timeout: 10000 });
});
